...
def contar_vocales(mi_string):
    mi_string = mi_string.lower()    #pasa a minusculas antes de ejecutar
    vocales = ('aeiou')
    resultado = {}
...
class TestContarVocales(unittest.TestCase):
    ...
    def test_contar_AnatOmIA(self):
        resultado = contar_vocales('AnatOmIA')
        self.assertEqual(resultado, {'a': 3, 'i': 1, 'o': 1})

    def test_contar_EUFORIA(self):
        resultado = contar_vocales('EUFORIA')
        self.assertEqual(resultado, {'a': 1, 'e': 1, 'i': 1, 'o': 1, 'u': 1})
    
    def test_contar_RIaCHUELO(self):
